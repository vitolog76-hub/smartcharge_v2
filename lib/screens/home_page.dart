...
class _HomePageState extends State<HomePage> {
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      setState(() { selectedCar = car; _capacityController.text = car.batteryCapacity.toString(); });
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  double finalCost = (tipo == "Home") ? CostCalculator.calculate(finalKwh, const TimeOfDay(hour: 23, minute: 0), selectedDate, myContract) : finalKwh * (double.tryParse(priceCtrl.text) ?? 0.65);
                  setState(() { chargeHistory.add(ChargeSession(date: selectedDate, kwh: finalKwh, cost: finalCost, location: tipo, startTime: TimeOfDay.now(), endTime: TimeOfDay.now())); });
                  _saveHistory(); Navigator.pop(context);
                },
  ...
  Widget _buildDialogTile(IconData icon, String label, String value, VoidCallback onTap) {
    return ListTile(leading: Icon(icon, color: Colors.blueAccent), title: Text(label, style: const TextStyle(color: Colors.white38, fontSize: 10)), subtitle: Text(value, style: const TextStyle(color: Colors.white)), onTap: onTap);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_isSimulating && _simService.scheduledStart != null) {
      startTimeDisplay = DateFormat('HH:mm').format(_simService.scheduledStart!);
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        backgroundColor: Colors.black,
        leading: IconButton(icon: const Icon(Icons.history, color: Colors.blueAccent), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => HistoryPage(history: chargeHistory, contract: myContract, selectedCar: selectedCar)))),
        actions: [IconButton(icon: const Icon(Icons.settings, color: Colors.blueAccent), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => SettingsPage(contract: myContract, selectedCar: selectedCar, batteryValue: _capacityController.text))))],
      ),
      body: SafeArea(
          ...
          child: Column(
            children: [
              GestureDetector(
                ...
                child: Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: const Color(0xFF1C1C1E), borderRadius: BorderRadius.circular(18), border: Border.all(color: _isSimulating ? (isChargingReal ? Colors.greenAccent : Colors.orangeAccent) : Colors.blueAccent.withOpacity(0.2))),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(_isSimulating ? (isChargingReal ? "RICARICA IN CORSO..." : "IN ATTESA...") : "AUTO PRONTA PER LE ORE:", style: TextStyle(color: _isSimulating ? (isChargingReal ? Colors.greenAccent : Colors.orangeAccent) : Colors.white38, fontSize: 9, fontWeight: FontWeight.bold)),
                        Text(readyTime.format(context), style: const TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.w900)),
                      ]),
                      Icon(_isSimulating ? (isChargingReal ? Icons.bolt_rounded : Icons.timer) : Icons.access_time, color: _isSimulating ? (isChargingReal ? Colors.greenAccent : Colors.orangeAccent) : Colors.blueAccent, size: 36),
                    ],
                  ),
                ),
              ),
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
                    if (_isSimulating) {
                      if (isChargingReal && currentSoc > _socAtStartOfSim) {
                        ...
                        showDialog(
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            actions: [
                              TextButton(onPressed: () { _simService.stopSimulation(); Navigator.pop(ctx); }, child: const Text("SCARTA", style: TextStyle(color: Colors.redAccent))),
                              ElevatedButton(
                                style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent),
                                onPressed: () { 
                      ...
                      // AVVIO SICURO: Salviamo il valore ESATTO prima di ogni altra cosa
              ...
              const SizedBox(height: 16),
              // CORREZIONE ERRORI NULL: Passiamo una funzione vuota () {}
              ActionButtons(
                  ...
                  Expanded(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(color: const Color(0xFF1C1C1E), borderRadius: BorderRadius.circular(15), border: Border.all(color: Colors.white10)),
                  ...
                  Container(
                    ...
                    child: Column(children: [
                      ...
                      TextField(
                        ...
                        decoration: const InputDecoration(isDense: true, border: InputBorder.none, suffixText: " kWh"),
                      ),
                    ]),
                  ),
                ],
